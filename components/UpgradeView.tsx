...
export default function UpgradeView({ user, allItems, onUpdateBalance }: Props) {
  ...
  const handleUpgrade = async () => {
    ...
    // МИ БІЛЬШЕ НЕ СКИДАЄМО КУТ ДО НУЛЯ! Це виправляє баг з відкатом назад.
    ...
    try {
      ...
      if (data.success) {
        // Розраховуємо новий кут відносно ПОТОЧНОЇ позиції стрілки
        const finalAngle = (data.roll / 100) * 360;
        
        setRollAngle(prevAngle => {
          // Дізнаємося, скільки повних обертів стрілка вже зробила раніше
          const currentSpins = Math.floor(prevAngle / 360);
          // Додаємо ще 6 обертів для напруги + фінальний кут
          return (currentSpins + 6) * 360 + finalAngle;
        });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center bg-gray-900/50 p-8 rounded-2xl border border-gray-800 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center relative z-10 w-full h-72">
          <div className="relative w-60 h-60 flex items-center justify-center">
            ...
            {/* Стрілка з оновленою анімацією */}
            <div 
              className="absolute inset-0 w-full h-full z-20 pointer-events-none"
              style={{ 
                transform: `rotate(${rollAngle}deg)`,
                transition: spinning ? 'transform 5.5s cubic-bezier(0.3, 0.0, 0.1, 1)' : 'none'
              }}
            >
            ...
            {/* Внутрішність кола */}
            <div className="z-10 flex flex-col items-center bg-gray-900 w-48 h-48 rounded-full justify-center shadow-inner border-4 border-gray-800 pointer-events-auto">
              {resultData ? (
                <div className="flex flex-col items-center animate-fade-in">
                  <div className={`text-3xl font-black tracking-widest uppercase ${resultData.isWin ? 'text-green-400 drop-shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'text-red-500 drop-shadow-[0_0_10px_rgba(239,68,68,0.5)]'}`}>
                    {resultData.isWin ? 'WIN!' : 'CRASH'}
                  </div>
                  
                  {/* ОНОВЛЕНА КНОПКА TRY AGAIN */}
                  <button 
                    onClick={() => setResultData(null)}
                    className="mt-4 text-[10px] font-bold text-gray-300 hover:text-white bg-gray-800 hover:bg-gray-700 px-5 py-2 rounded-full border border-gray-600 transition-colors uppercase tracking-widest shadow-md"
                  >
                    Try Again
                  </button>
                </div>
